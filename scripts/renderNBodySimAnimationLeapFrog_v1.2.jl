...
function calculate_precise_limits(data, iteration;
    ...
    if dynamic
        ...
        # Reutilizamos los límites de x de XY
        xz_xlims = xy_xlims  
...
#*                        xy_xlims=(-6,6), xy_ylims=(-6,6),
#*                        xz_xlims=(-4,6), xz_ylims=(-1,1),
#*                        yz_xlims=(-3,3), yz_ylims=(-1,1))
#&-------------------------------------------------------------------------

#&------Si prefieres límites dinámicos, podrías usar:----------------------
        limits = calculate_precise_limits(data, iteration; dynamic=true)
#&-------------------------------------------------------------------------

        xy_limits = limits.xy
        xz_limits = limits.xz
        yz_limits = limits.yz   # CORREGIDO: usar `yz` en lugar de `y`
